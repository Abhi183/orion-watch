...
// ── Constants ────────────────────────────────────────────────────────────────
pub const MU_EARTH: f64 = 398_600.4418; // km³/s²
...
impl StateVector {
    ...
    pub fn visviva_speed(&self) -> f64 {
        ...
        if inside < 0.0 { 0.0 } else { inside.sqrt() }
    }
...
pub fn parse_oem(content: &str) -> Result<ParsedOem> {
    let mut meta = OemMeta {
        object_name: String::new(), object_id: String::new(),
        ref_frame:   String::new(), time_system: String::new(),
        start_time:  String::new(), stop_time:   String::new(),
        creation:    String::new(), originator:  String::new(),
    };
    ...
    for line in data_content.lines() {
        let line = line.trim();
        if line.is_empty() || line.starts_with("CCSDS") { continue; }

        // Metadata
        if let Some(val) = kv(line, "OBJECT_NAME")  { meta.object_name = val; continue; }
        if let Some(val) = kv(line, "OBJECT_ID")    { meta.object_id   = val; continue; }
        if let Some(val) = kv(line, "REF_FRAME")    { meta.ref_frame   = val; continue; }
        if let Some(val) = kv(line, "TIME_SYSTEM")  { meta.time_system = val; continue; }
        if let Some(val) = kv(line, "START_TIME")   { meta.start_time  = val; continue; }
        if let Some(val) = kv(line, "STOP_TIME")    { meta.stop_time   = val; continue; }
        if let Some(val) = kv(line, "CREATION_DATE"){ meta.creation    = val; continue; }
        if let Some(val) = kv(line, "ORIGINATOR")   { meta.originator  = val; continue; }
...
pub fn mission_summary(oem: &ParsedOem, features: &[FeatureRow]) -> MissionSummary {
    let n   = features.len();
    let dur = (oem.vectors.last().unwrap().epoch - oem.vectors[0].epoch)
                .num_seconds() as f64 / 86400.0;

    let max_dist_idx = features.iter().enumerate()
        .max_by(|a,b| a.1.dist_km.partial_cmp(&b.1.dist_km).unwrap()).unwrap().0;
    let max_dist = features[max_dist_idx].dist_km;
    let apo_epoch = features[max_dist_idx].epoch.clone();

    let min_speed = features.iter().map(|f| f.speed_kms)
    ...
    let std_e  = (features.iter().map(|f| (f.energy - mean_e).powi(2)).sum::<f64>() / n as f64).sqrt();
...
pub fn write_features_csv(features: &[FeatureRow], path: &str) -> Result<()> {
    ...
    for row in features { wtr.serialize(row)?; }
    wtr.flush()?;
    Ok(())
}
...
// ── Math helpers ──────────────────────────────────────────────────────────────
#[inline] pub fn dot3(a: [f64; 3], b: [f64; 3]) -> f64 {
    a[0]*b[0] + a[1]*b[1] + a[2]*b[2]
}
#[inline] pub fn norm3(v: [f64; 3]) -> f64 {
    dot3(v, v).sqrt()
}
#[inline] pub fn unit3(v: [f64; 3]) -> [f64; 3] {
    let n = norm3(v).max(1e-30);
    [v[0]/n, v[1]/n, v[2]/n]
}
#[inline] pub fn cross3(a: [f64; 3], b: [f64; 3]) -> [f64; 3] {
